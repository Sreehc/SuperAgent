...
- 支持知识库、文档上传、解析、切块、向量化、删除和重处理。
- 支持向量检索、关键词检索、RRF 融合、可插拔 Rerank 和无证据兜底。
- 支持 Trace 查询、运行时设置、Agent 管理接口和治理能力。

当前仓库已包含 Agent 管理与治理相关能力，例如 Agent Runs、Tool Calls、Plugins、Tools 控制台和知识治理页；但完整的开放式 Agent 工具调用闭环仍处于持续演进中。
...
- Refresh Token 通过后端 HttpOnly Cookie 传输，前端不会在 `localStorage` 中保存长期凭证。
- 外部工具默认关闭，需要管理员显式配置后才可启用。
