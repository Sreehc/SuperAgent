...
| 4 | 工具协议、插件注册中心与租户级启停 | 已完成 | 完整 manifest 元数据、租户级绑定/密钥引用、插件治理接口、错误审计和控制台展示已收口 |
| 5 | 联网工具、HTTP 工具与受控 Python 执行 | 已完成 | `Tavily` 搜索、网页抓取抽取、HTTP allowlist/写方法限制、Sandbox 限制和高风险工具角色门禁均已打通 |
...
1. 当前 1-11 阶段已全部收口，后续重点转入性能、安全和上线硬化。
2. 评测门禁已经就位，后续每次改造都应继续通过 `agent-eval` 再推进。
3. 如果继续扩展 Agent 生态，建议优先补外部 MCP 兼容和更细粒度的工具授权模型。

当前已知阻塞记录：

- 当前没有新的阶段性硬阻塞，计划书中的完整 Agent 闭环已全部落地。
...
- `ToolSpec / ToolInvocation / ToolResult` 已成为统一工具协议
- `plugins/<plugin-key>/manifest.json` 已支持更完整的 schema、超时和重试元数据
- 启动扫描、插件注册、租户安装状态、绑定配置、密钥引用和参数模板已形成闭环
- 运行时仅向当前租户暴露可用工具，高风险工具仍受显式绑定和角色门禁限制
- `GET /api/v1/admin/plugins`、`PATCH /api/v1/admin/plugins/{pluginId}`、`GET /api/v1/admin/tool-calls` 已完成并补齐治理信息
- 前端 `Tools / Plugins` 控制台已展示启用工具、密钥引用和最近失败概览
- 新增 manifest 解析、插件启停可见性、治理接口和错误审计测试
...
- `web.search` 已接入 `Tavily` 适配器，并支持 provider 配置、API key 注入和结构化结果输出
- `web.fetch` 已支持真实页面抓取、标题/发布时间提取、正文清洗与摘要输出
- `http.request` 已支持 allowlist 校验、默认 `GET` 限制和租户级写方法授权
- `python.sandbox` 已接通 `sandbox-runner`，并补齐无外网、CPU/内存/文件输出限制元数据
- 高风险工具运行时已增加角色、租户、环境三级门禁
- 新增工具访问策略、搜索/抓取/allowlist/Sandbox 相关自动化测试
...
- `MemoryStrategy` 四种策略已完整接入：
  - `NONE`
  - `SLIDING_WINDOW`
  - `SUMMARY_WINDOW`
  - `SUMMARY_PLUS_WINDOW`
- `conversation_memory_summary` 已成为真实摘要存储，并按消息覆盖范围递增版本
- 超过阈值消息后会自动生成新摘要，而不是无限拼接原始长上下文
- `SUMMARY_WINDOW` 在摘要缺失时会回退到最近窗口，避免上下文为空
- Agent 默认记忆策略已验证为 `SUMMARY_PLUS_WINDOW`

本阶段验证：

- `cd /Users/cheers/Desktop/workspace/SuperAgent/backend && ./mvnw -Dtest=ConversationMemoryServiceTest,RuntimeSettingsServiceTest test`
...
- 已新增固定回放集定义文件：`eval/agent-eval-suite.json`
- 已新增本地与 CI 可复用的评测 runner：`scripts/agent_eval.py`
- 已新增 GitHub Actions workflow：`.github/workflows/agent-eval.yml`
- 回放集已覆盖：
  - `CLARIFICATION`
  - 经典 RAG 流式问答
  - 无证据 RAG 回归
...
- `python3 /Users/cheers/Desktop/workspace/SuperAgent/scripts/agent_eval.py`
